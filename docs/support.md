...
**winprint** writes extensive diagnostic logs to `%appdata%/Kindel Systems/WinPrint/logs`. When using the PowerShell command line (`out-winprint`) specify `-Debug` to send diagnostic log entries to the console as well as the log file. When using `winprint.cli`, specify `--debug`.
