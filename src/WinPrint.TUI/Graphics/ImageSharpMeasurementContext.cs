...
public sealed class ImageSharpMeasurementContext : IGraphicsContext, IDisposable
{
    ...
    public IGraphicsState Save() => _inner.Save();
    public void Restore(IGraphicsState state) => _inner.Restore(state);
    public void TranslateTransform(float dx, float dy) => _inner.TranslateTransform(dx, dy);
    public void ScaleTransform(float sx, float sy) => _inner.ScaleTransform(sx, sy);
    public void SetClip(GraphicsRectF rect) => _inner.SetClip(rect);
    public void ExcludeClip(GraphicsRectF rect) => _inner.ExcludeClip(rect);
    public void ResetClip() => _inner.ResetClip();
    public void SetTextRenderingMode(GraphicsTextRenderingMode mode) => _inner.SetTextRenderingMode(mode);

    public IGraphicsFont CreateFont(string family, float size, GraphicsFontStyle style, GraphicsFontUnit unit)
        => _inner.CreateFont(family, size, style, unit);

    public IGraphicsBrush CreateSolidBrush(GraphicsColor color) => _inner.CreateSolidBrush(color);
    public IGraphicsPen CreatePen(GraphicsColor color, float width = 1f) => _inner.CreatePen(color, width);

    public GraphicsSizeF MeasureString(string text, IGraphicsFont font) => _inner.MeasureString(text, font);

    public GraphicsSizeF MeasureString(string text, IGraphicsFont font, int width, GraphicsStringFormat format)
        => _inner.MeasureString(text, font, width, format);
    ...
    public GraphicsSizeF MeasureString(string text, IGraphicsFont font, GraphicsSizeF proposedSize,
        ...
        => _inner.MeasureString(text, font, proposedSize, format, out charsFitted, out linesFilled);
